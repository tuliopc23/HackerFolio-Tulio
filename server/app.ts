        ...
        switch (command) {
          ...
          case 'projects': {
            ...
            const header = ansi.dim(
              `Page ${effPage}/${totalPages} • Showing ${start + 1}-${end} of ${total}`
            )
            const hints: string[] = []
            if (effPage > 1)
              hints.push(
                `${ansi.magenta('Prev')}: ${ansi.cyan(`projects ${baseFlags} --page ${effPage - 1}`)}`
              )
            if (effPage < totalPages)
              hints.push(
                `${ansi.magenta('Next')}: ${ansi.cyan(`projects ${baseFlags} --page ${effPage + 1}`)}`
              )
          ...
          case 'about': {
            const row = (
              await orm
                .select({ content: tContent.content })
                .from(tContent)
          ...
          case 'skills': {
            ...
            return {
              output: `${ansi.magenta(ansi.bold('Skills'))}\n${ansi.dim('------')}\n${items}`,
            }
          }
          case 'contact': {
            const row = (
            ...
            const sep = '-----------------------'
            return {
              output: `${ansi.magenta(ansi.bold('Contact'))}\n${ansi.dim(sep)}\n${lines.join('\n')}`,
            }
          }
          case 'github': {
            ...
            return {
              output: `${ansi.magenta(ansi.bold('GitHub'))}\n${ansi.dim('------')}\n${ansi.magenta('Profile')}: ${ansi.underline(ansi.cyan(String(profile)))}`,
              action: { type: 'open_url', url: String(profile) },
            }
          }
          case 'resume': {
            const foundCommand = found as { responseTemplate?: string }
            ...
            if (typeof url === 'string' && url.startsWith('http')) {
              return {
                output: `${ansi.magenta(ansi.bold('Resume'))}\n${ansi.dim('------')}\n${ansi.magenta('URL')}: ${ansi.underline(ansi.cyan(url))}`,
          ...
          case 'grep': {
            ...
            if (sub === 'stack') {
              ...
              const items = list.length
                ...
                : ansi.yellow('-')
              return {
                output: `${ansi.magenta(ansi.bold('Stack'))}\n${ansi.dim('-----')}\n${items}`,
              }
            }
            return {
              output: ansi.red(`grep: '${args?.join(' ')}' - try 'grep stack'`),
              error: true,
            }
          }
          case 'whoami': {
            ...
            return {
              output: `${ansi.magenta(ansi.bold('Profile'))}\n${ansi.dim('-------')}\n${lines.join('\n')}`,
            }
          }
          case 'stack': {
            const row = (
