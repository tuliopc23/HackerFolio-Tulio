...
class HotReloadCoordinator {
  ...
  private startClientWatcher(): void {
    ...
    const watcher = watch(clientPath, { recursive: true }, (_, filename) => {
  ...
  private startServerWatcher(): void {
    ...
    const watcher = watch(serverPath, { recursive: true }, (_, filename) => {
      if (!filename || this.shouldIgnoreFile(filename)) return
  ...
  private startSharedWatcher(): void {
    ...
    const watcher = watch(sharedPath, { recursive: true }, (_, filename) => {
      if (!filename || this.shouldIgnoreFile(filename)) return
  ...
  private async executeReloadStrategy(changes: Map<string, ChangeEvent[]>): Promise<void> {
    ...
    for (const priority of priorities) {
      for (const [key, events] of changes) {
        ...
        if (!firstEvent || firstEvent.priority !== priority) continue
  ...
  private getServerAction(_filename: string): 'reload' | 'restart' | 'refresh' {
  ...
  private reportStats(): void {
    ...
    console.log(colorize(`  • Last change: ${uptimeMinutes}m ago`, 'blue'))
    console.log('')
  }
