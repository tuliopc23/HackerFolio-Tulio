...
class PathMappingValidator {
  ...
  validateTypeScriptPaths(): void {
    ...
    try {
      ...
      if (tsBaseUrl !== expectedBaseUrl) {
        this.addIssue(`TypeScript baseUrl mismatch: found "${tsBaseUrl}", expected "${expectedBaseUrl}"`)
      } else {
  ...
  private validatePathMappings(tool: string, actualPaths: PathMapping, expectedPaths: PathMapping): void {
    ...
    expectedAliases.forEach(alias => {
      if (actualPaths[alias]) {
        const actualPath = Array.isArray(actualPaths[alias]) ? actualPaths[alias][0] : actualPaths[alias]
        const expectedPath = Array.isArray(expectedPaths[alias]) ? expectedPaths[alias][0] : expectedPaths[alias]
        
        if (actualPath !== expectedPath) {
          this.addIssue(`${tool} path mismatch for ${alias}: found "${actualPath}", expected "${expectedPath}"`)
        } else {
    ...
    if (this.result.issues.length > 0) {
      ...
      this.result.issues.forEach(issue => {
        ...
        if (issue.includes('TypeScript baseUrl')) {
          console.log(colorize('    → Run: bun run server/scripts/generate-tsconfig.ts --update', 'blue'))
        }
        
        if (issue.includes('Vite configuration should import')) {
          console.log(colorize('    → Update vite.config.ts to import from ./shared/config', 'blue'))
        }
        
        if (issue.includes('Vitest configuration should import')) {
          console.log(colorize('    → Update vitest.config.ts to import from ./shared/config', 'blue'))
        }
      })
    }
