...
import { config, generateConfigReport, validateCrossConfiguration } from '../../shared/config'
  ...
  validateTypeScriptConfiguration(): void {
    ...
    try {
      ...
      if (tsBaseUrl === centralTsPaths.baseUrl) {
        ...
        this.addWarning(
          `TypeScript baseUrl (${tsBaseUrl}) differs from central config (${centralTsPaths.baseUrl})`
        )
      }
  ...
  validateDevelopmentWorkflow(): void {
    ...
    try {
      ...
      if (devConfig.proxyPort === appConfig.port) {
        ...
        this.addIssue(
          `Proxy port (${devConfig.proxyPort}) does not match app port (${appConfig.port})`
        )
      }
  ...
  validateProductionReadiness(): void {
    ...
    try {
      ...
      if (appConfig.environment === 'production') {
        ...
        if (
          securityConfig.corsOrigins.length > 0 &&
